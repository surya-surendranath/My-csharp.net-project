...
public partial class SummaryByFaculty : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["LoginAccepted"] != null)
        {
            if (Session["FacultyYear"] != null && Session["FacultyMonth"] != null)
            {
                ...
                if (dr.Read())
                {
                    TitleTXT.Text = "Overall Monthly Summary Report on all Faculties for the month of " + dr["Monthname"] + " in Year " + TxbFacultyYear.Text;
                }
                else
                {
