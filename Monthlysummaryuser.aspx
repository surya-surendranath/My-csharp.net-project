<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Monthlysummaryuser.aspx.cs" Inherits="Monthlysummaryuser" %>
...
&nbsp;</p>
    ...
    <p style="text-align: center">
        <asp:Button ID="Button1" runat="server" OnClick="Button1_Click" Text="Ok" Width="76px" style="margin-bottom: 124px" />
    </p>
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        SelectCommand="SELECT GrantType, DATENAME(MONTH, DateOfIssue) AS month_name, COUNT(DISTINCT student_ID) AS NoofStudents, COUNT(student_ID) AS NoofGrants, SUM(GrantValue) AS Amount, SUM(ISNULL(KuhaFunds, 0)) AS KohafundsAmount, SUM(GrantValue) + SUM(ISNULL(KuhaFunds, 0)) AS TotalGrantAmount FROM Student_vouchers WHERE (YEAR(DateOfIssue) = @Year) AND (MONTH(DateOfIssue) = @monthnumber) AND (GrantType = @Voucher) GROUP BY GrantType, DATENAME(MONTH, DateOfIssue)">
        <SelectParameters>
